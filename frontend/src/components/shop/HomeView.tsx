...
const BRAND_IMAGES: Record<string, string> = {
  uniqlo:
    "https://images.unsplash.com/photo-1571867424488-4565932edb41?auto=format&fit=crop&q=80&w=800",
        ...
        {isLoadingBrands ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
            {brands.slice(0, 8).map((brand) => (
              <div
                key={brand.id}
                className="group relative h-[200px] sm:h-[200px] lg:h-[250px] rounded-3xl sm:rounded-[2.5rem] overflow-hidden cursor-pointer shadow-md hover:shadow-2xl transition-all duration-700 hover:-translate-y-1"
                onClick={() => onBrandSelect(brand.id)}
              >
                <img
                  src={
                    brand.logo_url ||
                    BRAND_IMAGES[brand.name.toLowerCase()] ||
                    BRAND_IMAGES.default
                  }
                  alt={brand.name}
